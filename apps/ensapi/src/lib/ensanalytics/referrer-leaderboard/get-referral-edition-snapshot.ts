...
/**
 * Builds a {@link ReferralEditionSnapshot} from the database using the provided referral program rules.
 *
 ...
 * @param accurateAsOf - The {@link UnixTimestamp} of when the data used to build the {@link ReferralEditionSnapshot} was accurate as of.
 * @throws Error if the database query fails
 */
export async function getReferralEditionSnapshot(
  ...
  switch (rules.awardModel) {
    case ReferralProgramAwardModels.PieSplit: {
      ...
      return buildReferralEditionSnapshotPieSplit(allReferrers, rules, accurateAsOf);
    }
    case ReferralProgramAwardModels.RevShareCap: {
      const events = await getReferralEvents(rules);
      return buildReferralEditionSnapshotRevShareCap(events, rules, accurateAsOf);
    }
    case ReferralProgramAwardModels.Unrecognized:
      ...
      throw new Error(
        `getReferralEditionSnapshot called with unrecognized award model '${rules.originalAwardModel}' — edition should have been filtered before reaching this point.`,
      );

    default: {
      const _exhaustiveCheck: never = rules;
      throw new Error(
        `Unexpected award model in getReferralEditionSnapshot: ${(_exhaustiveCheck as ReferralProgramRules).awardModel}`,
      );
    }
  }
}

...
export default function () {
  ...
  ponder.on(
    namespaceContract(pluginName, "Ethnames_BaseRegistrarOld:ControllerAdded"),
    async ({ context, event }) => {
  ...
  ponder.on(
    namespaceContract(pluginName, "Ethnames_BaseRegistrarOld:ControllerRemoved"),
    async ({ context, event }) => {
  ...
  ponder.on(
    namespaceContract(pluginName, "Ethnames_BaseRegistrar:ControllerAdded"),
    async ({ context, event }) => {
      await handleControllerAddedToRegistrar(
        context,
        buildEventRef({
  ...
  ponder.on(
    namespaceContract(pluginName, "Ethnames_BaseRegistrar:ControllerRemoved"),
    async ({ context, event }) => {
      await handleControllerRemovedFromRegistrar(
        context,
        buildEventRef({

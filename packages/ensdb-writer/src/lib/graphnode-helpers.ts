...
export async function labelByLabelHash(
  ...
  labelHash: LabelHash,
): Promise<LiteralLabel | null> {
  const { ensRainbow } = context;
  ...
  try {
    response = await pRetry(
      async () => {
        ...
        const result = await ensRainbow.heal(labelHash);
    ...
    if (error instanceof Error) {
      error.message = `ENSRainbow Heal Request Failed: ENSRainbow unavailable at '${ensRainbow.getOptions().endpointUrl}'.`;
    }

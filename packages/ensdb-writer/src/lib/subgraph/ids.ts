import type { Address, LabelHash, Node } from "enssdk";
 ...
 *
 * @param isSubgraphCompatible whether to use subgraph-compatible ID formatting
...
export const makeResolverId = (
  ...
  [
    ...
    isSubgraphCompatible ? null : chainId,
    address,
    node,
  ]
    .filter(Boolean)
...
export const makeRegistrationId = (
  ...
  if (isSubgraphCompatible) return labelHash;
  return node;
};

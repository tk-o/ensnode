...
export async function handleBridgedResolverChange(
  ...
  const nextBridged = nextResolver
    ? isBridgedResolver(context.namespace, { chainId: registry.chainId, address: nextResolver })
    : null;
  ...
  const prevBridged = domain?.subregistryId
    ? isBridgedTargetRegistry(context.namespace, domain.subregistryId)
    : null;
...
async function reconcileRegistryCanonicality(
  ...
  if (isRootRegistryId(context.namespace, registryId)) return;

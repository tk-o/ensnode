...
import type { EventWithArgs, IndexingEngineContext, LogEventBase } from "../../../../types";
...
async function getRegistrarAndRegistry(context: IndexingEngineContext, event: LogEventBase) {
  ...
  if (accountIdEqual(registrar, ensv2ETHRegistrar)) {
    return {
      registrar,
      registry: getDatasourceContract(context.namespace, DatasourceNames.ENSv2Root, "ETHRegistry"),
    };
  }

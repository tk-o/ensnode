...
import type { EventWithArgs, IndexingEngineContext, IndexingEngineLogger } from "../../../../types";
...
export default function (adapter: IndexingEngineAdapter) {
  async function handleNameRegisteredByController({
    ...
    const label = literalLabelIfMatchesLabelHash(context.logger, event.args.label, labelHash);

    const controller = getThisAccountId(context, event);
    const { node: managedNode, registry } = getManagedName(context.namespace, controller);

...
import {
  ...
  ensureUnknownLabel,
  labelExists,
} from "../../../../lib/ensv2/label-db-helpers";
...
export default function (adapter: IndexingEngineAdapter) {
  ...
  async function handleNewOwner({
    ...
    if (!exists) {
      ...
      if (
        ...
        context.chain.id === getENSRootChainId(context.namespace) &&
        ...
        context.namespace !== ENSNamespaceIds.SepoliaV2
      ) {
    ...
    // NOTE: Canonicalize ENSv1Registry vs. ENSv1RegistryOld via `getManagedName(context.namespace, ...).registry`.
    ...
    if (isTLD) {
      ...
      if (created) {
        ...
        if (isBridgedOriginDomain(context.namespace, parentDomainId)) {
        ...
        const bridged = isBridgedTargetRegistry(context.namespace, parentRegistryId);
        if (bridged) {
   ...
   * - Lineanames Registry
   */
  adapter.on(namespaceContract(pluginName, "ENSv1Registry:NewOwner"), handleNewOwner);

...
export default function (adapter: IndexingEngineAdapter) {
  ...
  adapter.on(
    ...
    async ({
      ...
      try {
        ...
        context.logger.warn({ msg: `NameWrapper emitted malformed DNSEncodedName: '${name}'` });
      }
  ...
  adapter.on(namespaceContract(pluginName, "NameWrapper:TransferSingle"), handleTransfer);
  adapter.on(

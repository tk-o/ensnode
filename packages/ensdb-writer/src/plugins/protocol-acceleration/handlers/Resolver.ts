...
export default function (adapter: IndexingEngineAdapter) {
  adapter.on(namespaceContract(pluginName, "Resolver:AddrChanged"), async ({ context, event }) => {
  ...
  adapter.on(namespaceContract(pluginName, "Resolver:NameChanged"), async ({ context, event }) => {
    const key = await ensureResolverAndRecords(context, event);

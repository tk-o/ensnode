...
import { pluginSupportsPremintedNames } from "../../../lib/namespace-contract";
import {
  sharedEventValues,
  upsertAccount,
  upsertRegistration,
...
import { handleNewOwner } from "./Registry";
...
export const makeRegistrarHandlers = ({ pluginName }: { pluginName: PluginName }) => {
  async function setNamePreimage(
    ...
    if (context.isSubgraphCompatible && !isLabelSubgraphIndexable(label)) return;

    const interpretedLabel = context.isSubgraphCompatible
    ...
    await context.ensDb
      .update(ensIndexerSchema.subgraph_registration, {
        id: makeRegistrationId(context.isSubgraphCompatible, labelHash, node),
      })
  ...
  return {
    ...
    async handleNameRenewed({
      ...
      const { node: managedNode } = getManagedName(
        context.namespace,
        getThisAccountId(context, event),
      );
      const node = makeSubdomainNode(labelHash, managedNode);
      const id = makeRegistrationId(context.isSubgraphCompatible, labelHash, node);

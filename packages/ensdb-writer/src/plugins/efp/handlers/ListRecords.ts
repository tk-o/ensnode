...
export default function (adapter: IndexingEngineAdapter) {
  ...
  adapter.on(namespaceContract(pluginName, "ListRecords:ListOp"), async ({ context, event }) => {
    ...
    switch (parsed.opcode) {
      ...
      case EFP_OPCODE.REMOVE_TAG: {
        ...
        await context.ensDb
          .update(ensIndexerSchema.efpListRecords, { id })
          .set({ tags: record.tags.filter((existing: string) => existing !== tagOp.tag) });
        return;
      }

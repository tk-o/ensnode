...
The Docker images are the easiest way to run or deploy the ENSNode suite of services, both locally and in the cloud. All [ENSNode releases](https://github.com/namehash/ensnode/releases) include Docker images for the core ENSNode services:

- [ENSRainbow](https://github.com/namehash/ensnode/pkgs/container/ensnode%2Fensrainbow)
- [ENSIndexer](https://github.com/namehash/ensnode/pkgs/container/ensnode%2Fensindexer)
- [ENSApi](https://github.com/namehash/ensnode/pkgs/container/ensnode%2Fensapi)
...
Edit the file you just created (`docker/envs/.env.docker.local`) to set your `NAMESPACE`, `PLUGINS`, and RPC endpoints (e.g. `ALCHEMY_API_KEY` or `RPC_URL_1`), then run:
...
Configuration is optional. To customize defaults (e.g. change `PLUGINS`), copy the example as in the previous step and edit it.
